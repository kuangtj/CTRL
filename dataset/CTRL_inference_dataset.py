...
with open('./data/DSP_data_from_TDC/dsp_smiles_dict.json', 'r', encoding='utf-8') as f:
    dsp_smiles_dict = json.load(f)
...
def get_dataset(model_type=None, task_name=None):
...
#            frac = [0.98, 0.01, 0.01]
            ...
            for idx in range(len(test_smiles_list)):
                if not os.path.exists(f"./data_store/DTI_mol_pt/mol_{str(test_molname_list[idx]) + str(dti_smiles_dict[test_smiles_list[idx]].replace('/', '_')[:245 - len(str(test_molname_list[idx]))])}.pt"):
                    ...
                    continue
                if not os.path.exists(f"./data_store/DTI_prot_pt/prot_{dti_prot_dict[test_ps_list[idx]]}.pt"):
                    print("error test data path: ", f"./data_store/DTI_prot_pt/prot_{dti_prot_dict[test_ps_list[idx]]}.pt")
                    continue
                item = {}
                item['protein_rep'] = f"./data_store/DTI_prot_pt/prot_{dti_prot_dict[test_ps_list[idx]]}.pt"
            ...
            test_data_items_pro = test_data_items_list
...
class MolTestDatasetWrapper(object):
    ...
    def get_data_loaders(self, model_type, task_name):
        test_dataset_list = get_dataset(model_type, task_name)
        test_loader = self.get_train_validation_data_loaders(test_dataset_list, model_type)
        return test_loader
        

    def get_train_validation_data_loaders(self, test_dataset_list, model_type):

        test_dataset = MoleculeDataset(test_dataset_list, model_type)
