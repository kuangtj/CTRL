...
def get_dataset(task_id_want):
    ...
    file_path = f'./data/DDI_data_from_TDC/{task_id_want}_data.json'
    ...
    for idx in range(len(smiles_list)):
        try:
            ...
            data_item = smiles_preporcess(mol_smiles, f"./data_store/DDI_mol_sdf/mol_{mol_smiles.replace('/', '_')[:245]}.sdf")
            torch.save(data_item, f"./data_store/DDI_mol_pt/mol_{mol_smiles.replace('/', '_')[:245]}.pt")
