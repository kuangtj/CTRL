...
from dataset.CTRL_finetune_second_stage_dataset import MolTestDatasetWrapper
import math
apex_support = False
try:
...
class FineTune(object):
    ...
    def train(self):
        ...
        optimizer = torch.optim.Adam(
            [{'params': pred_params, 'lr': self.config['lr'] - 0.01 * self.config['lr'] * self.config['warm_up']}],
            self.config['init_lr'], weight_decay=self.config['weight_decay']
        )

        for n, p in model.named_parameters():
            if 'pred_heads' not in n:
        ...
        for epoch_counter in range(self.config['warm_up'], self.config['epochs']):
            import time
            ...
            for bn, [data_2D, data_3D1, data_3D2, morgan_fp, maccs_fp, daylight_fp, descriptor_int, descriptor_float, src_mask, mask_3d, prot_rep, prot_num, cellline_drp_rep, cellline_dsp_rep, is_catalyst, tasks, labels] in enumerate(nb_train_loader):
                if (bn + 0) % 10 == 0:
                    print("----------", bn, "----------")
                    print("lr: ", f"{optimizer.param_groups[0]['lr']:.8f}")
                optimizer.zero_grad()
                ...
                if True:
                    loss, _, tmp_loss_list = self._step2(model, data_2D, data_3D1, data_3D2, morgan_fp, maccs_fp, daylight_fp, descriptor_int, descriptor_float, src_mask, mask_3d, prot_rep, prot_num, cellline_drp_rep, cellline_dsp_rep, is_catalyst, tasks, labels, epoch_counter=epoch_counter)
            ...
            optimizer.param_groups[0]['lr'] = optimizer.param_groups[0]['lr'] - 0.01 * self.config['lr']
            ...
            for loss_key, loss_value in all_tmp_loss_list.items():
                ...
                valid_loss, valid_results = self._validate(model, valid_loader, epoch_counter=epoch_counter)
                print('Valid loss:', valid_loss)
                print('Valid result: ', valid_results)
                valid_loss_list.append(valid_loss)
                test_loss, test_results = self._validate(model, test_loader, epoch_counter=epoch_counter, test=True)
                ...
                torch.save(model.state_dict(), f"./ckpt4/{self.config['model_type']}/epoch_{epoch_counter}_weight.pth")

                valid_n_iter += 1

    def _validate(self, model, valid_loader, epoch_counter=1000, test=False):
...
def main(config):
    dataset = MolTestDatasetWrapper(config['batch_size'], config['dataset']['num_workers'])
